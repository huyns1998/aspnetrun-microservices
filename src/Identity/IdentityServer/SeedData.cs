    ...
    public class SeedData
    {
        ...
        private static async void EnsureUsers(IServiceScope scope)
        {
            ...
            if (alice == null)
            {
                ...
                {
                     Task.WaitAll(roleMgr.CreateAsync(new IdentityRole(roleName)));
                }
                ...
                var result = userMgr.CreateAsync(alice, "Pa55w0rd!").Result;

                if (!result.Succeeded)
                {
                    throw new Exception(result.Errors.First().Description);
                }
                Task.WaitAll(userMgr.AddToRoleAsync(alice, "Admin"));
          ...
          new Claim(JwtClaimTypes.WebSite, "http://alice.com"),
            ...
            if (bob == null)
            {
                ...
                var result = userMgr.CreateAsync(bob, "Pa55w0rd!").Result;
                if (!result.Succeeded)
                {
                    throw new Exception(result.Errors.First().Description);
                }
                Task.WaitAll(userMgr.AddToRoleAsync(bob, roleName));
